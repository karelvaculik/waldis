...
graph = DynamicGraph.from_json_file("data_graphs/enron_graph.json.gz", gz=True)


positive_edge_label = 2
negative_edge_label = 5
n = 20

# time_unit_primary = 48 * 3600
# time_unit_secondary = 48 * 3600 / 4.0

data_name = "enron"
param_time_unit_primary = [96 * 3600, 48 * 3600, 48 * 3600, 48 * 3600, 48 * 3600]
param_time_unit_secondary = [48 * 3600, 48 * 3600, 12 * 3600, 12 * 3600, 12 * 3600]
...
for i in range(len(param_time_unit_primary)):
    ...
    for repetition in range(num_of_repetitions):
        ...
        # TRAIN DATA
        positive_starting_times = positive_vertex_timestamps.min(axis=0) - 1
        negative_starting_times = negative_vertex_timestamps.min(axis=0) - 1
